...
class DBcontroller:
    def set(self, key: str, value, path: str = config["sep"]):
    ...
    def delete(self, key: str, path: str = config["sep"]):
    ...
    def db_list(
        self, path: str = config["sep"], sortbydirs: bool = False, removeprefix: bool = True
    ):
        ...
        for i in intermediate:
            if sortbydirs:
                ...
                if dir not in result_list:
                    result_list.append(dir[:-1] if self.db_list(path + dir) == [] else dir)
    ...
    def drop(self, path: str = config["sep"]):
        "drop all stored keys within a path"
        valcheck(root(path))
    
        for i in self.db_list(path, False, False):
            del db[msg + i]
    
    
    def reference(self, method: str, input, path: str = config["sep"]):
    ...
    def read(self, key: str, path: str = config["sep"]):
